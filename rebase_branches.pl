...
say "\nNot koha-plugin-edifact-thorndike, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-thorndike";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-thorndike';
    next if $repo eq 'koha-plugin-edifact-thorndike-docs';
